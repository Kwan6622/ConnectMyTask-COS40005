...
class ApiService {
  ...
  tasks = {
    ...
    verifyOnSite: (id: string | number) => apiClient.post(`/tasks/${id}/verify-onsite`),
  };
  ...
  tracking = {
    update: (data: { taskId: number; providerId: number; lat: number; lng: number; timestamp: string }) => apiClient.post("/tracking/update", data),
  };
